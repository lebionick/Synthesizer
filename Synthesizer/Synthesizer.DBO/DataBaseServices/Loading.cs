...
namespace Synthesizer.DBO
{
    ...
    class LoadGuitar : ILoadSound
    {
        List<string> ILoadSound.Load(int octavas, ref string status)
        {
            List<string> _soundList = new List<string>();
            status = "Все звуки успешно загружены";
            ...
            for (int i = 0; i < octavas * 12; i++)
            {
                string path = ("..\\..\\..\\SoundBanks\\guitartones\\guitartone-" + Convert.ToString(i + 1) + ".wav");
                if (File.Exists(@path))
                    _soundList.Add(path);
                else
                {
